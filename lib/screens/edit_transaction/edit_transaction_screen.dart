...
class _EditTransactionScreenState extends State<EditTransactionScreen> {
  ...
  void didChangeDependencies() {
    ...
    _loadTransaction(args.transactionId);
  }
  ...
  Future<void> _loadTransaction(String id) async {
    try {
      final transaction = await _repository.fetchTransaction(id);
      if (!mounted) return;
      if (transaction == null) {
        setState(() => _isLoading = false);
        return;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: _isLoading || _original == null
          ? null
          : SafeArea(
              top: false,
              child: Padding(
